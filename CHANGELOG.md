...
- [#1](https://github.com/awangs1986/catskills/pull/1) [`da6fc3a`](https://github.com/awangs1986/catskills/commit/da6fc3a76593c2d4dbbd305e4ec00fb98f414da7) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Add the `test-audit` skill (engineering bucket, model-invoked). Audits the tests behind a change for whether they protect the business logic or only pass: translates each test into a one-sentence claim in `CONTEXT.md` vocabulary for the domain expert to judge (flagging name/assertion mismatches, tests that claim nothing, and tautological assertions), maps claims to acceptance criteria and user stories in both directions with untested wrong paths listed, and runs a targeted mutation probe (ten to fifteen mutants at business-rule sites, one at a time, tree verified clean between each) to find rules no test kills. Audits, never fixes; survivors and uncovered rows return to `tdd` as named red tests. `implement` now calls it after `verify` and before `code-review`, and ends with a **Checks run** ledger listing what was actually checked.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`da6fc3a`](https://github.com/awangs1986/catskills/commit/da6fc3a76593c2d4dbbd305e4ec00fb98f414da7) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Add the `verify` skill (engineering bucket, model-invoked). Boots the built thing and walks each acceptance criterion and user story as the actor would (headless browser, curl, the CLI, a direct trigger), tries one obvious wrong path per criterion, and reports PASS / FAIL / UNVERIFIABLE with a file of evidence per row. Observes, never fixes. `implement` now calls it after the suite is green and before `code-review`, and takes each FAIL back into the `tdd` loop.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`da6fc3a`](https://github.com/awangs1986/catskills/commit/da6fc3a76593c2d4dbbd305e4ec00fb98f414da7) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Add the `vibe` skill (engineering bucket, user-invoked). A dispatcher for one developer working alone: it puts the user on one of four lanes (Build, Fix, Review, Tidy), sizes Build work as S / M / L with three ordered questions, and hands back a route card naming the exact next command, the steps after it, and the context rule between them. It routes and stops; it may fire a model-invoked first step (`tdd`, `diagnosing-bugs`, `code-review`) only on an explicit "go". The skill ships with `WORKFLOW.md`, the human-facing map it reads from: the curated sixteen-skill kit, the skills deliberately left out with the moment to bring each back, per-lane walkthroughs, five context rules, what accumulates in the repo, two worked sessions, and a cheatsheet. `ask-matt` now points solo users at it and takes them back when other people enter the picture.

- [#3](https://github.com/awangs1986/catskills/pull/3) [`2ca08bc`](https://github.com/awangs1986/catskills/commit/2ca08bcbfa70f701f94d81cb894f139fd9e6ad45) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Give every skill a portable, synchronized conversation policy: a gentle, attentive secretary-like manner and the literal `喵！` marker at user-facing paragraph boundaries, without decorating commands, quotes, structured output, or technical artifacts. Carry the rule into setup-generated project instructions and promoted docs, soften adversarial interview wording while retaining the full decision process, and check synchronization mechanically across all buckets.

  Audit Vibe's guidance against every promoted skill. Add an explicit route-coverage inventory, route standalone guides and session care before setup, distinguish product stories, acceptance-case design, runtime verification, and test auditing, and correct the stale claim that research is outside the solo kit.

  Refresh Askcat's inventory, picker, and offline template: deduplicate installs, label repository catalogs honestly, include the storyteller and varied cat coats, validate cards and picker edges against the discovered inventory, keep copyable examples clean, and safely serialize source text into the single HTML file. Add regression tests for routing coverage, policy portability, page validation, and the rendered guide.

  Fix the SVG mascot rendering so each role shows only its own accessories, and embed a two-glyph, OFL-licensed font so the fixed conversation marker also renders offline without system CJK fonts. Preserve progress for the open page when browser storage is blocked.

  Run the repository and guide regression checks on pushes and pull requests, and before the release workflow prepares a version update.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`101ca4e`](https://github.com/awangs1986/catskills/commit/101ca4e58888476b3876902b1d5fc7a63e2f1989) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - The plugin is now `cat-skills`, in this repo's own marketplace `awangs1986` (`/plugin marketplace add awangs1986/popcodeskills`, then `/plugin install cat-skills@awangs1986`); it no longer shares a name with the upstream `mattpocock-skills` listing. `package.json`, `plugin.json`, `marketplace.json`, the changeset config and every pending changeset carry the new name. Docs pages now link to each other and into the repo with repo-relative paths instead of `aihero.dev/skills-<name>` URLs, which only exist for upstream's skills; `check-skills` fails on a docs link that does not resolve.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`da6fc3a`](https://github.com/awangs1986/catskills/commit/da6fc3a76593c2d4dbbd305e4ec00fb98f414da7) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - `vibe`: when invoked with no argument and in-flight work exists, hand back a where-you-were block (branch, tickets with status, last commits, newest refocus brief, next ticket on the frontier) before asking the lane question; route through `/setup-feedback-loops` when `docs/agents/feedback-loops.md` is missing. The workflow handbook brings `research` back into the kit for library and API questions, adds `setup-feedback-loops` as the second setup step, and threads `verify` and `security-review` through the Build and Review lanes.

### Patch Changes

- [#926](https://github.com/mattpocock/skills/pull/926) [`84b5ee5`](https://github.com/awangs1986/catskills/commit/84b5ee5afd738b6a3484e62509b84b3b573c5be3) Thanks [@mattpocock](https://github.com/mattpocock)! - Add the `implement-spec` skill (in-progress bucket, user-invoked). It takes a spec and its tickets and drives them to a single PR: the tickets are read as a task graph with blocking edges, so implementer subagents run in background worktrees across the ready frontier for concurrency, a merger subagent folds each one back into the PR branch, and the flow closes with `/code-review` before the PR is marked ready.

- [#1092](https://github.com/mattpocock/skills/pull/1092) [`d75dcf1`](https://github.com/awangs1986/catskills/commit/d75dcf1c5bccfab736ab79f494fa248980546d45) Thanks [@mattpocock](https://github.com/mattpocock)! - Add the `pr` skill (in-progress bucket, model-invoked). It's a reference for the shape a pull request body should take, not a workflow: the template comes first, then a short section per part of it. The summary comes from the primary source (the issue or spec), never inferred from the diff; the body states size and a one-way/two-way door call up front; "the shape of the change" reproduces `show-me` almost verbatim (credited in the skill's `CREDITS.md`), aimed at a diff instead of a conversation; evidence is a before/after pair (visual first, a failing-then-passing test run where no visual exists); and what was deliberately left out gets its own section. Relates to [#521](https://github.com/awangs1986/popcodeskills/issues/521), [#938](https://github.com/awangs1986/popcodeskills/issues/938), [#509](https://github.com/awangs1986/popcodeskills/issues/509), and [#915](https://github.com/awangs1986/popcodeskills/issues/915).

- [#848](https://github.com/mattpocock/skills/pull/848) [`f02e2ed`](https://github.com/awangs1986/catskills/commit/f02e2ed3624d031272f8547742d23bf6bca8b072) Thanks [@mattpocock](https://github.com/mattpocock)! - domain-modeling: trigger on discussing codebase terminology and on writing or editing a CONTEXT.md or an ADR directly, replacing the narrower "pin down domain terminology or a ubiquitous language" / "record an architectural decision" phrasing. Also drops the "another skill needs to maintain the domain model" caveat, since that's the invoking skill's job to state explicitly, not this description's.

- [#911](https://github.com/mattpocock/skills/pull/911) [`4f28947`](https://github.com/awangs1986/catskills/commit/4f289474bad013fe2be8f8769d733f59d9103d6b) Thanks [@mattpocock](https://github.com/mattpocock)! - Quote the `description` front matter in `to-spec`, `code-review`, `setup-matt-pocock-skills`, `writing-fragments`, `writing-shape`, and `wait-what`. An unquoted colon-space left over from the em-dash sweep in [#905](https://github.com/awangs1986/popcodeskills/issues/905) made each block invalid YAML, so `skills.sh` skipped all six during discovery and they couldn't be listed or installed via `npx skills`.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`3460bcb`](https://github.com/awangs1986/catskills/commit/3460bcb34bd4f647396763631450ba8d3b8698ec) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Install block rewritten for this fork (clone and link, skills.sh against `awangs1986/popcodeskills`, the repo's own marketplace as the documented plugin route) so `README.md` copies it verbatim again. New `npm run check-skills` (`scripts/check-skills.mjs`) checks the skill invariants in `CLAUDE.md` mechanically: promoted skills present in both READMEs, the bucket README, `plugin.json`, a docs page with the fixed frame, `openai.yaml` agreeing with the frontmatter, `ask-matt` naming every user-invoked skill, no em-dashes, English only outside the translation. `implement` now names the `test-cases.md` sheet `verify` walks and reports it in the Checks run block; `cattytest` no longer duplicates the sheet as a tracker issue; `askcat`, `takeover` and `wait-what` docs pages gain their missing sections; `CLAUDE.md` names the Pi link directory.

- [#917](https://github.com/mattpocock/skills/pull/917) [`85f83d3`](https://github.com/awangs1986/catskills/commit/85f83d3fde1d3a90d5c9a657f6998c79a6c37308) Thanks [@mattpocock](https://github.com/mattpocock)! - grilling: update the round template so consecutive questions are separated by a horizontal rule (`---`) instead of running together.

- [#879](https://github.com/mattpocock/skills/pull/879) [`d419977`](https://github.com/awangs1986/catskills/commit/d419977fe07d9e1607d3523f3579310bbb076b93) Thanks [@mattpocock](https://github.com/mattpocock)! - grilling: remove em-dashes from `SKILL.md`, replacing them with colons and semicolons so the instructions read as plain text.

- [#1](https://github.com/awangs1986/catskills/pull/1) [`da6fc3a`](https://github.com/awangs1986/catskills/commit/da6fc3a76593c2d4dbbd305e4ec00fb98f414da7) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - `implement` now claims its ticket at the start (`Status: in-progress` locally; assign and comment on GitHub/GitLab) and closes it at the end (`Status: done` plus the commit sha and its Checks run block under `## Comments`; close or label on a real tracker), ticking the acceptance criteria it met. The local-markdown tracker doc documents the three-state implementation ticket lifecycle and defines the frontier in terms of it, so `/vibe`'s where-you-were block and "next ticket on the frontier" have something real to read. The `vibe` handbook adds docs-drift reconciliation to the Tidy lane and a section on `retro` as the loop that closes on the environment rather than the product.

- [#3](https://github.com/awangs1986/catskills/pull/3) [`2ca08bc`](https://github.com/awangs1986/catskills/commit/2ca08bcbfa70f701f94d81cb894f139fd9e6ad45) Thanks [@arena-ai-coding-agent](https://github.com/apps/arena-ai-coding-agent)! - Refresh the English and Chinese workflow posters with the optional `tell-a-story` product-alignment on-ramp before setup: explicit narrator choices, iterative story confirmation, and optional product SPEC / BACKLOG drafts. Update the curated-kit count to 26 skills and keep setup scoped to publishing and building.

  Replace the six orange-only role mascots with a coordinated set of distinct coats, and add a calico storyteller with a storybook. Use transparent cutouts to preserve white fur, share all seven mascots across both posters and README galleries, and document the reproducible render and font setup.

- [#905](https://github.com/mattpocock/skills/pull/905) [`e6e9577`](https://github.com/awangs1986/catskills/commit/e6e957797d8cceb5b351c0dc840369523f9fb8fb) Thanks [@mattpocock](https://github.com/mattpocock)! - Remove every em-dash from the repo's prose (docs, `SKILL.md` files, ADRs, `README.md`, scripts, JSON/YAML metadata), hand-rewriting each sentence with a comma, colon, period, parentheses, or conjunction rather than mechanically substituting the character. `CLAUDE.md`/`AGENTS.md` now says not to reintroduce them.

- [#1083](https://github.com/mattpocock/skills/pull/1083) [`6942bff`](https://github.com/awangs1986/catskills/commit/6942bff8a53666e83a2c621696d2a8e3022429e6) Thanks [@mattpocock](https://github.com/mattpocock)! - retro: classify coding-standards findings as mechanical or judgement calls before writing them. A mechanical violation (a fixed syntactic pattern, a banned API, an import shape, a file-location rule) now gets a deterministic check instead (a linter rule, a pre-commit hook, or a CI job), reserving `CODING_STANDARDS.md` for genuine judgement calls. Automated checks also now flags a repo with no guardrail at all (no pre-commit hook, no CI lint/typecheck/test job) as a finding in its own right.

- [#878](https://github.com/mattpocock/skills/pull/878) [`e3e547b`](https://github.com/awangs1986/catskills/commit/e3e547b57d549110a0aa6ff40fd7b871c01c76c9) Thanks [@mattpocock](https://github.com/mattpocock)! - Standardize cross-skill invocation on an explicit "call the Skill tool" instruction instead of bare `/skill`-style prose, across `code-review`, `diagnosing-bugs`, `grill-with-docs`, `grill-me`, `improve-codebase-architecture`, `tdd`, `to-spec`, `to-tickets`, `triage`, and `wayfinder`.
...
- [#880](https://github.com/mattpocock/skills/pull/880) [`1dab982`](https://github.com/awangs1986/catskills/commit/1dab98299c3b81f560026c01b7ebf55ed5d91373) Thanks [@mattpocock](https://github.com/mattpocock)! - Stop skills from trying to reach user-invoked skills through the Skill tool: fix cross-skill references that violated the "no other skill can call it" invariant in `.agents/invocation.md`, in `to-spec`, `wayfinder`, `to-tickets`, `triage`, `code-review`, and `diagnosing-bugs`.

  - `to-spec`, `wayfinder`, `to-tickets`, `triage`, and `code-review` each carried a precondition ("...run `/setup-matt-pocock-skills` if not") that PR [#878](https://github.com/awangs1986/popcodeskills/issues/878) rewrote into a literal `Call the Skill tool with "setup-matt-pocock-skills"` instruction. `setup-matt-pocock-skills` is user-invoked, so none of these skills (user-invoked or model-invoked) can call it. Reworded all five as instructions for the agent to tell the human to run it instead.
  ...
  - Added a carve-out paragraph to `.agents/invocation.md`'s "Dependencies between them" section: the `Call the Skill tool with "name"` convention only applies when the named skill is model-invoked. This is the section PR [#878](https://github.com/awangs1986/popcodeskills/issues/878) introduced without reconciling it against the user-invoked/model-invoked invariant stated eight lines above it; the gap is most of why this bug reached six call sites instead of one.

  Fixes [#453](https://github.com/awangs1986/popcodeskills/issues/453).

- [#904](https://github.com/mattpocock/skills/pull/904) [`594f0f8`](https://github.com/awangs1986/catskills/commit/594f0f83188921a60d45d63d6cdac509de20df2c) Thanks [@mattpocock](https://github.com/mattpocock)! - wait-what: follow `CONTEXT-MAP.md` to the right `CONTEXT.md` when a repo indexes multiple contexts that way instead of keeping a single root `CONTEXT.md`.
